...
class Loader(BasicDataset):
    ...
    def __init__(self, config=world.config, path="../data/gowalla"):
        ...
        self.Graph, self.Graph_UI, self.Graph_U2, self.Graph_I2 = None, None, None, None
    ...
    def getSparseGraph(self):
        ...
        if self.Graph is None or self.Graph_UI is None:
            try:
                norm_adj = sp.load_npz(self.path + '/s_pre_adj_mat.npz')
                norm_adj_r = sp.load_npz(self.path + '/s_pre_adj_mat_r.npz')
                norm_adj_u2 = sp.load_npz(self.path + '/s_pre_adj_mat_u2.npz')
                norm_adj_i2 = sp.load_npz(self.path + '/s_pre_adj_mat_i2.npz')
                ...
                norm_adj_u2 = norm_adj_r.dot(norm_adj_r.T)
                norm_adj_u2.setdiag(0)
                norm_adj_i2 = norm_adj_r.T.dot(norm_adj_r)
                norm_adj_i2.setdiag(0)
            ...
            if self.split == True:
                ...
                self.Graph_UI = self.Graph_UI.coalesce().to(world.device)

                self.Graph_U2 = self._convert_sp_mat_to_sp_tensor(norm_adj_u2)
                self.Graph_U2 = self.Graph_U2.coalesce().to(world.device)

                self.Graph_I2 = self._convert_sp_mat_to_sp_tensor(norm_adj_i2)
                self.Graph_I2 = self.Graph_I2.coalesce().to(world.device)
